...
def shuffle_in_unison_scary(a, b, c, d, e, f, g):
...
def getDataFromTxt(txt, with_landmark=True):
    ...
    for line in lines:
        ...
        if len(components) == 19:
            ...
            if int(components[15]) == 1:
                gender = [1,0]
            else:
                gender = [0,1]
                all_attr[0] = 1

            if int(components[16]) == 1:
                smile = [1,0]
            else:
                smile = [0,1]
                all_attr[1] = 1

            if int(components[17]) == 1:
                glasses = [1,0]
            else:
                glasses = [0,1]
                all_attr[2] = 1

            if int(components[18]) == 1:
                pose = [1,0,0,0,0]
                all_attr[3] = 1
            elif int(components[18]) == 2:
                pose = [0,1,0,0,0]
                all_attr[4] = 1
            elif int(components[18]) == 3:
                pose = [0,0,1,0,0]
                all_attr[5] = 1
            elif int(components[18]) == 4:
                pose = [0,0,0,1,0]
                all_attr[6] = 1
            elif int(components[18]) == 5:
                pose = [0,0,0,0,1]
                all_attr[7] = 1
        else:
            gender, smile, glasses, pose, all_attr = -1
        
        result.append((img_path, BBox(bbox), landmark, gender, smile, glasses, pose, all_attr))
