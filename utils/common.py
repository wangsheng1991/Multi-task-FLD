...
def shuffle_in_unison_scary(a, b, c, d, e, f):
...
def getDataFromTxt(txt, with_landmark=True):
    ...
    for line in lines:
        ...
        if len(components) == 19:
            gender = int(components[15])-1
            smile = int(components[16])-1
            glasses = int(components[17])-1
            pose = int(components[18])-1
        else:
            gender, smile, glasses, pose = -1, -1, -1, -1
        
        result.append((img_path, BBox(bbox), landmark, gender, smile, glasses, pose))
